...
static ssize_t do_io(int fd, OriginFun fun, const char* hook_fun_name,
	...
	if(!sylar::t_hook_enable) {
		return fun(fd, std::forward<Args>(args)...);			//将参数展开给originFun
	}
	sylar::FdCtx::ptr ctx = sylar::FdMgr::GetInstance()->get(fd);
	//如果不存在，则不是socket，使用原函数
	if(!ctx) {
		return fun(fd, std::forward<Args>(args)...);
	}
	//如果存在但被关闭，设置errno badf
	if(ctx->isClose()) {
		errno = EBADF;
		return -1;
	}
	...
	uint64_t to = ctx->getTimeout(timeout_so);					//timeout_so是type
	std::shared_ptr<timer_info> tinfo(new timer_info);

retry:
	ssize_t n = fun(fd, std::forward<Args>(args)...);			//执行原来的方法
	...
	if(n == -1 && errno == EAGAIN) {							//阻塞
		...
		if(to != (uint64_t)-1) {
			timer = iom->addConditionTimer(to, [winfo, fd, iom, event]() {
					auto t = winfo.lock();						//强化成shared_ptr 
...
#undef XX
	...
	int socket(int domain, int type, int protocol) {
		...
		sylar::FdMgr::GetInstance()->get(fd, true);				//查询，不存在则创建
		return fd;
	}
	...
	int close(int fd) {
		...
		if(ctx) {
			...
			if(iom) {
				iom->cancelAll(fd);							//所有事件都取消
			}
			sylar::FdMgr::GetInstance()->del(fd);			//删除fd
		}
		return close_f(fd);									//再用原来方法关闭
	}
